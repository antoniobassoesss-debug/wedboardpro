...
import { snapToGrid, snapLineToAngle, calculateDistance, calculateAngle } from '../utils/gridSnap.js';
// Helper function to calculate distance between two points
const calculateDistanceBetweenPoints = (p1, p2) => {
    return Math.sqrt(Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2));
};
const WallMaker = ({ width, height, config, walls, onWallsChange, onClose, onAddToCanvas, onSavePreset, activeTool = 'wall', doors = [], onDoorsChange, }) => {
    ...
    const [angleSnapInfo, setAngleSnapInfo] = useState(null);
    const [selectedWall, setSelectedWall] = useState(null);
    const [history, setHistory] = useState([walls || []]);
    const [historyIndex, setHistoryIndex] = useState(0);
    const [hoveredPoint, setHoveredPoint] = useState(null);
    const [isPanning, setIsPanning] = useState(false);
    const [panStart, setPanStart] = useState(null);
    const [doorDrawingMode, setDoorDrawingMode] = useState('idle');
    const [doorHinge, setDoorHinge] = useState(null);
    const [doorEnd, setDoorEnd] = useState(null);
    ...
    const saveToHistory = useCallback((newWalls) => {
    ...
    const findNearestEndpoint = useCallback((point, threshold = 10) => {
    ...
    const findWallAtPoint = useCallback((point, threshold = 10) => {
        let closest = null;
        walls.forEach(wall => {
    ...
    const handleMouseDown = useCallback((e) => {
        ...
        if (activeTool === 'door') {
            ...
            if (doorSelection) {
                ...
                if (dist <= arcRadius + 30) {
                    ...
                    if (clickedDirection) {
                        ...
                        const wallLength = Math.sqrt(Math.pow(wall.endX - wall.startX, 2) + Math.pow(wall.endY - wall.startY, 2));
            ...
            if (doorDrawingMode === 'idle') {
                ...
                let finalEndX, finalEndY, finalEndPos;
                ...
                const doorWidth = Math.sqrt(Math.pow(finalEndX - doorHinge.x, 2) + Math.pow(finalEndY - doorHinge.y, 2));
                ...
                if (doorWidth >= 20) {
                    ...
                    const endSide = dotProduct >= 0 ? 'right' : 'left';
                    // Determine which end of the door opening is the hinge (start or end along the frame)
                    const hingeSide = doorHinge.position <= finalEndPos ? 'start' : 'end';
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        // Handle wall drawing
        if (activeTool !== 'wall' || (drawingMode !== 'start-set' && drawingMode !== 'preview'))
            return;
        if (!currentStart)
            return;
        const point = screenToSvg(e.clientX, e.clientY);
        ...
        if (config.snapAngles.length > 0) {
            const angleSnap = snapLineToAngle(currentStart.x, currentStart.y, endPoint.x, endPoint.y, config.snapAngles);
    ...
    const handleWallRightClick = useCallback((e, wall) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const renderGrid = () => {
        ...
        return (_jsxs("defs", { children: [_jsx("pattern", { id: "wall-maker-grid-pattern", x: "0", y: "0", width: gridSizePx, height: gridSizePx, patternUnits: "userSpaceOnUse", children: _jsx("path", { d: `M ${gridSizePx} 0 L 0 0 0 ${gridSizePx}`, fill: "none", stroke: "#d0d0d0", strokeWidth: "0.5", opacity: "0.5" }) }), _jsx("pattern", { id: "wall-maker-major-grid-pattern", x: "0", y: "0", width: majorGridSizePx, height: majorGridSizePx, patternUnits: "userSpaceOnUse", children: _jsx("path", { d: `M ${majorGridSizePx} 0 L 0 0 0 ${majorGridSizePx}`, fill: "none", stroke: "#b0b0b0", strokeWidth: "1", opacity: "0.6" }) })] }));
    };
    ...
    const renderWalls = () => {
        return walls.map(wall => {
            ...
            if (wallDoors.length > 0) {
                ...
                return (_jsxs("g", { children: [segments.map((seg, idx) => {
                            ...
                            return (_jsx("line", { x1: segStartX, y1: segStartY, x2: segEndX, y2: segEndY, stroke: isSelected ? '#3498db' : '#2c3e50', strokeWidth: wall.thickness, strokeLinecap: "round", cursor: "pointer", onContextMenu: (e) => handleWallRightClick(e, wall), style: { filter: isSelected ? 'drop-shadow(0 0 4px rgba(52, 152, 219, 0.5))' : 'none' } }, `segment-${idx}`));
                        }), config.showMeasurements && (_jsxs("text", { x: (wall.startX + wall.endX) / 2, y: (wall.startY + wall.endY) / 2 - 10, fontSize: "12", fill: "#666666", textAnchor: "middle", pointerEvents: "none", children: [wall.length ? pixelsToMeters(wall.length).toFixed(2) : '0', "m"] })), config.showAngles && wall.angle !== undefined && (_jsxs("text", { x: wall.endX + 10, y: wall.endY, fontSize: "10", fill: "#999999", pointerEvents: "none", children: [wall.angle.toFixed(0), "\u00B0"] }))] }, wall.id));
            }
            // Wall without doors - render normally
            return (_jsxs("g", { children: [_jsx("line", { x1: wall.startX, y1: wall.startY, x2: wall.endX, y2: wall.endY, stroke: isSelected ? '#3498db' : '#2c3e50', strokeWidth: wall.thickness, strokeLinecap: "round", cursor: "pointer", onContextMenu: (e) => handleWallRightClick(e, wall), style: { filter: isSelected ? 'drop-shadow(0 0 4px rgba(52, 152, 219, 0.5))' : 'none' } }), config.showMeasurements && (_jsxs("text", { x: (wall.startX + wall.endX) / 2, y: (wall.startY + wall.endY) / 2 - 10, fontSize: "12", fill: "#666666", textAnchor: "middle", pointerEvents: "none", children: [wall.length ? pixelsToMeters(wall.length).toFixed(2) : '0', "m"] })), config.showAngles && wall.angle !== undefined && (_jsxs("text", { x: wall.endX + 10, y: wall.endY, fontSize: "10", fill: "#999999", pointerEvents: "none", children: [wall.angle.toFixed(0), "\u00B0"] }))] }, wall.id));
        });
    };
    // Render current drawing line (preview)
    const renderCurrentLine = () => {
        if (drawingMode !== 'preview' || !currentStart || !currentEnd)
            return null;
        ...
        return (_jsxs("g", { children: [_jsx("line", { x1: currentStart.x, y1: currentStart.y, x2: currentEnd.x, y2: currentEnd.y, stroke: isSnapped ? '#27ae60' : '#3498db', strokeWidth: config.defaultThickness, strokeLinecap: "round", strokeDasharray: "5,5", opacity: 0.5 }), config.showMeasurements && (_jsxs("text", { x: (currentStart.x + currentEnd.x) / 2, y: (currentStart.y + currentEnd.y) / 2 - 10, fontSize: "12", fill: isSnapped ? '#27ae60' : '#3498db', textAnchor: "middle", fontWeight: "bold", pointerEvents: "none", opacity: 0.8, children: [pixelsToMeters(length).toFixed(2), "m"] })), config.showAngles && (_jsxs("text", { x: currentEnd.x + 10, y: currentEnd.y, fontSize: "10", fill: isSnapped ? '#27ae60' : '#3498db', fontWeight: "bold", pointerEvents: "none", opacity: 0.8, children: [angle.toFixed(0), "\u00B0 ", angleSnapInfo?.snapped ? '✓' : ''] }))] }));
    };
    ...
    const renderDoors = () => {
        return doors.map(door => {
            ...
            const frameEndPoint = hingeSide === 'start'
                ? { x: doorEndX, y: doorEndY }
                : { x: doorStartX, y: doorStartY };
            const baseAngle = Math.atan2(frameEndPoint.y - hingePoint.y, frameEndPoint.x - hingePoint.x) * 180 / Math.PI;
            const radius = Math.sqrt(Math.pow(frameEndPoint.x - hingePoint.x, 2) + Math.pow(frameEndPoint.y - hingePoint.y, 2));
            ...
            return (_jsxs("g", { children: [_jsx("path", { d: swingPath, fill: "#000000", opacity: "0.15" }), _jsx("line", { x1: doorStartX, y1: doorStartY, x2: doorEndX, y2: doorEndY, stroke: "#ffffff", strokeWidth: wall.thickness + 4, strokeLinecap: "round", opacity: "1" }), _jsx("path", { d: `M ${arcStartX} ${arcStartY} A ${radius} ${radius} 0 ${largeArcFlag} ${sweepFlag} ${arcEndX} ${arcEndY}`, fill: "none", stroke: "#000000", strokeWidth: "4", strokeLinecap: "round", opacity: "0.6" }), _jsx("line", { x1: doorStartX, y1: doorStartY, x2: doorEndX, y2: doorEndY, stroke: "#000000", strokeWidth: "3", strokeLinecap: "round" }), _jsx("circle", { cx: hingePoint.x, cy: hingePoint.y, r: "3", fill: "#000000", stroke: "#ffffff", strokeWidth: "1.5", opacity: "0.8" })] }, door.id));
        }).filter(Boolean);
    };
    // Render door drawing preview (hinge to end point - only before second click)
    const renderDoorPreview = () => {
        // Only show preview if doorSelection is not set (i.e., before second click finalizes)
        if (!doorHinge || !doorEnd || doorDrawingMode === 'idle' || doorSelection)
            return null;
        const wall = walls.find(w => w.id === doorHinge.wallId);
        if (!wall)
            return null;
        ...
        return (_jsxs("g", { children: [_jsx("line", { x1: startX, y1: startY, x2: endX, y2: endY, stroke: "#ffffff", strokeWidth: wall.thickness + 4, strokeLinecap: "round", strokeDasharray: "5,5", opacity: "0.7" }), _jsx("line", { x1: startX, y1: startY, x2: endX, y2: endY, stroke: "#000000", strokeWidth: "3", strokeLinecap: "round", strokeDasharray: "5,5", opacity: "0.5" }), _jsx("circle", { cx: startX, cy: startY, r: "5", fill: "#000000", stroke: "#ffffff", strokeWidth: "2", opacity: "0.8" }), _jsx("circle", { cx: endX, cy: endY, r: "5", fill: "#000000", stroke: "#ffffff", strokeWidth: "2", opacity: "0.8" }), doorWidth > 0 && (_jsxs("text", { x: (startX + endX) / 2, y: (startY + endY) / 2 - 10, fontSize: "11", fill: "#000000", textAnchor: "middle", opacity: "0.6", fontWeight: "500", children: [pixelsToMeters(doorWidth).toFixed(2), "m"] }))] }));
    };
    ...
    const renderDoorDirectionOptions = () => {
        ...
        return (_jsxs("g", { children: [directions.map(({ dir, sweep, label }) => {
                    const startAngle = baseAngle;
                    const endAngle = baseAngle + sweep;
                    ...
                    return (_jsxs("g", { onMouseEnter: () => setHoveredDirection(dir), onMouseLeave: () => setHoveredDirection(null), children: [_jsx("path", { d: `M ${hingeX} ${hingeY} L ${arcStartX} ${arcStartY} A ${radius} ${radius} 0 ${largeArcFlag} ${sweepFlag} ${arcEndX} ${arcEndY} Z`, fill: "#000000", opacity: isHovered ? '0.25' : '0.15', style: { cursor: 'pointer' } }), _jsx("path", { d: `M ${arcStartX} ${arcStartY} A ${radius} ${radius} 0 ${largeArcFlag} ${sweepFlag} ${arcEndX} ${arcEndY}`, fill: "none", stroke: "#000000", strokeWidth: isHovered ? '5' : '4', strokeLinecap: "round", opacity: isHovered ? '0.8' : '0.6', style: { cursor: 'pointer' } }), _jsx("text", { x: labelX, y: labelY, fontSize: "14", fill: "#000000", textAnchor: "middle", dominantBaseline: "middle", opacity: isHovered ? '0.9' : '0.6', fontWeight: isHovered ? 'bold' : 'normal', pointerEvents: "none", children: label })] }, dir));
                }), _jsx("circle", { cx: hingeX, cy: hingeY, r: "4", fill: "#000000", stroke: "#ffffff", strokeWidth: "1.5", opacity: "0.9" })] }));
    };
    ...
    const renderSnapHighlight = () => {
        ...
        if (snapHighlight) {
            highlights.push(_jsx("circle", { cx: snapHighlight.x, cy: snapHighlight.y, r: 6, fill: "#27ae60", stroke: "#ffffff", strokeWidth: 2, opacity: 0.9, pointerEvents: "none" }, "snap-highlight"));
        }
        // Show all wall endpoints as small circles
        walls.forEach(wall => {
            const isStartSelected = selectedWall?.id === wall.id;
            highlights.push(_jsx("circle", { cx: wall.startX, cy: wall.startY, r: 4, fill: isStartSelected ? '#3498db' : '#2c3e50', stroke: "#ffffff", strokeWidth: 1.5, opacity: 0.8, pointerEvents: "none" }, `endpoint-start-${wall.id}`));
            highlights.push(_jsx("circle", { cx: wall.endX, cy: wall.endY, r: 4, fill: isStartSelected ? '#3498db' : '#2c3e50', stroke: "#ffffff", strokeWidth: 1.5, opacity: 0.8, pointerEvents: "none" }, `endpoint-end-${wall.id}`));
        });
        // Show start point circle when drawing
        if (currentStart && (drawingMode === 'start-set' || drawingMode === 'preview')) {
            highlights.push(_jsx("circle", { cx: currentStart.x, cy: currentStart.y, r: 5, fill: "#3498db", stroke: "#ffffff", strokeWidth: 2, opacity: 1, pointerEvents: "none" }, "current-start"));
        }
        // Show end point preview circle
        if (currentEnd && drawingMode === 'preview') {
            const isSnapped = snapHighlight !== null;
            highlights.push(_jsx("circle", { cx: currentEnd.x, cy: currentEnd.y, r: isSnapped ? 6 : 5, fill: isSnapped ? '#27ae60' : '#3498db', stroke: "#ffffff", strokeWidth: 2, opacity: 0.7, pointerEvents: "none" }, "current-end"));
        }
        return _jsx("g", { children: highlights });
    };
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            // Only handle if not typing in an input field
            if (!isInputField && (((e.metaKey || e.ctrlKey) && e.shiftKey && (e.key === 'z' || e.key === 'Z')) ||
    ...
    return (_jsxs("div", { style: {
            ...
            minHeight: '600px'
                    ...
                    cursor: activeTool === 'pan'
                ...
                }, onWheel: handleWheel, children: _jsxs("svg", { ref: svgRef, width: "100%", height: "100%", style: {
                        display: 'block',
                        background: '#ffffff',
                        position: 'absolute',
                        top: 0,
                        ...
                        width: '100%',
                        height: '100%'
                    }, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseLeave, children: [renderGrid(), _jsxs("g", { transform: `translate(${pan.x}, ${pan.y}) scale(${zoom})`, children: [config.showGrid && (_jsx("rect", { x: "-10000", y: "-10000", width: "20000", height: "20000", fill: "url(#wall-maker-grid-pattern)" })), config.showGrid && (_jsx("rect", { x: "-10000", y: "-10000", width: "20000", height: "20000", fill: "url(#wall-maker-major-grid-pattern)" })), _jsx("rect", { x: 0, y: 0, width: canvasWidth, height: canvasHeight, fill: "transparent", stroke: "#cccccc", strokeWidth: "1", strokeDasharray: "5,5", opacity: "0.5" }), renderWalls(), renderDoors(), renderCurrentLine(), renderDoorPreview(), renderDoorDirectionOptions(), renderSnapHighlight()] })] }) }), selectedWall && (_jsxs("div", { style: { padding: '12px', borderTop: '1px solid #e0e0e0', background: '#f9f9f9' }, children: [_jsx("div", { style: { marginBottom: '8px', fontWeight: '600', fontSize: '14px' }, children: "Wall Properties" }), _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '8px' }, children: [_jsxs("div", { style: { display: 'flex', gap: '8px' }, children: [_jsx("label", { style: { fontSize: '12px', width: '80px' }, children: "Length (m):" }), _jsx("input", { type: "number", value: selectedWall.length ? pixelsToMeters(selectedWall.length).toFixed(2) : '', onChange: (e) => {
                                        ...
                                        }, style: { flex: 1, padding: '4px 8px', border: '1px solid #e0e0e0', borderRadius: '4px', fontSize: '12px' } })] }), _jsxs("div", { style: { display: 'flex', gap: '8px' }, children: [_jsx("label", { style: { fontSize: '12px', width: '80px' }, children: "Angle:" }), _jsx("input", { type: "number", value: selectedWall.angle?.toFixed(0) || '', onChange: (e) => {
                                        ...
                                        }, style: { flex: 1, padding: '4px 8px', border: '1px solid #e0e0e0', borderRadius: '4px', fontSize: '12px' } })] }), _jsxs("div", { style: { display: 'flex', gap: '8px' }, children: [_jsx("label", { style: { fontSize: '12px', width: '80px' }, children: "Thickness (m):" }), _jsx("input", { type: "number", value: pixelsToMeters(selectedWall.thickness).toFixed(3), onChange: (e) => {
                                        ...
                                        }, style: { flex: 1, padding: '4px 8px', border: '1px solid #e0e0e0', borderRadius: '4px', fontSize: '12px' } })] }), _jsx("button", { onClick: () => handleDeleteWall(selectedWall.id), style: {
                                    marginTop: '8px',
                                    padding: '6px 12px',
                                    border: 'none',
                                    borderRadius: '6px',
...
//# sourceMappingURL=WallMaker.js.map