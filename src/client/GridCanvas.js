...
const getWallsBoundingBox = (wallsList) => {
    if (!wallsList || wallsList.length === 0)
        return null;
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
...
const derivePxPerMeterFromWalls = (wallsList) => {
    for (const wall of wallsList) {
        ...
        if (originalLengthPx && originalLengthPx > 0) {
            ...
            if (originalMeters > 0) {
                const actualLengthPx = Math.sqrt(Math.pow(wall.endX - wall.startX, 2) +
...
const computeWallScaleFromWalls = (wallsList) => {
...
const GridCanvas = forwardRef(({ activeTool, onToolChange, projectId, projectData, onDataChange, brushSize = 2, brushColor = '#000000' }, ref) => {
    ...
    const [drawings, setDrawings] = useState(projectData?.drawings || []);
    const [shapes, setShapes] = useState(projectData?.shapes || []);
    const [textElements, setTextElements] = useState(projectData?.textElements || []);
    const [walls, setWalls] = useState(projectData?.walls || []);
    const [doors, setDoors] = useState(projectData?.doors || []);
    ...
    const wallScaleRef = useRef(null);
    const movingShapeIdRef = useRef(null);
    const moveOffsetRef = useRef(null);
    const movingPositionRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const onKeyDown = (e) => {
            ...
            const isInputField = activeEl && (activeEl.tagName === 'INPUT' ||
    ...
    const addSpace = useCallback((widthMeters, heightMeters) => {
    ...
    const addTable = useCallback((type, size, seats, imageUrl, targetSpaceId) => {
        ...
        img.onload = () => {
            ...
            if ((chosenSpace || wallLayoutContext) && (widthMeters > 0 || heightMeters > 0)) {
                ...
                const ppm = chosenSpace
                    ? (chosenSpace.pixelsPerMeter
                        || Math.min(chosenSpace.width / Math.max(0.0001, (chosenSpace.spaceMetersWidth || 1)), chosenSpace.height / Math.max(0.0001, (chosenSpace.spaceMetersHeight || 1))))
            ...
            try {
                ...
                let spaceToUse = chosenSpace || currentSpaceRef.current || shapesRef.current.find(isSpaceShape);
    ...
    const addWalls = useCallback((newWalls, newDoors = []) => {
        ...
        const scaledWalls = newWalls.map(wall => {
        ...
        const scaledDoors = newDoors.map(door => ({
            ...door,
    ...
    const isPointOnCanvas = useCallback((x, y) => {
    ...
    useEffect(() => {
        ...
        window.addEventListener('toolbar:eraser-size', onEraserSize);
        return () => window.removeEventListener('toolbar:eraser-size', onEraserSize);
    }, []);
    ...
    const startMove = useCallback((shapeId, clientX, clientY) => {
        ...
        try {
            window.dispatchEvent(new CustomEvent('canvas:move-start'));
        }
        catch { }
    }, [screenToSvg]);
    ...
    const drawCanvas = useCallback(() => {
        ...
        doors.forEach(door => {
            ...
            const frameEndPoint = hingeSide === 'start'
                ? { x: doorEndX, y: doorEndY }
                : { x: doorStartX, y: doorStartY };
            const radius = Math.sqrt(Math.pow(frameEndPoint.x - hingePoint.x, 2) +
            ...
            const directions = door.openingDirection === 'both'
                ? ['left', 'right']
            ...
            directions.forEach(dir => {
                ...
                wedge.setAttribute('d', `M ${hingePoint.x} ${hingePoint.y} L ${arcStartX} ${arcStartY} A ${radius} ${radius} 0 0 ${sweepFlag} ${arcEndX} ${arcEndY} Z`);
                wedge.setAttribute('fill', '#000000');
                wedge.setAttribute('opacity', '0.15');
                svg.appendChild(wedge);
                const arc = document.createElementNS("http://www.w3.org/2000/svg", "path");
                arc.setAttribute('d', `M ${arcStartX} ${arcStartY} A ${radius} ${radius} 0 0 ${sweepFlag} ${arcEndX} ${arcEndY}`);
            ...
            if (shape.imageUrl) {
                ...
                image.addEventListener('click', (ev) => {
                    try {
                        ...
                        try {
                            window.dispatchEvent(new CustomEvent('canvas:selection-change', { detail: { selectedId: shape.id } }));
                        }
                        catch { }
                    }
                    catch (err) {
    ...
    const handleCanvasClick = useCallback((e) => {
        ...
        selectedShapeIdRef.current = null;
        try {
            window.dispatchEvent(new CustomEvent('canvas:selection-change', { detail: { selectedId: null } }));
        }
        catch { }
    }, []);
    ...
    const eraserPendingRef = useRef(null);
    const eraserRafRef = useRef(null);
    const scheduleErase = useCallback((x, y) => {
    ...
    const handleMouseMove = useCallback((e) => {
        ...
        if (movingShapeIdRef.current) {
            ...
            try {
                ...
                const movingId = movingShapeIdRef.current;
                if (svg) {
                    const el = svg.querySelector(`[data-shape-id="${movingId}"]`);
                    if (el) {
    ...
    const handleMouseUp = useCallback((e) => {
        ...
        if (movingShapeIdRef.current) {
            ...
            if (!stillExists) {
                ...
                isUserInteractingRef.current = false;
                try {
                    window.dispatchEvent(new CustomEvent('canvas:move-end'));
                }
                catch { }
                return;
            }
    ...
    useEffect(() => {
        ...
        window.addEventListener('trash:drop', onTrashDrop);
        return () => {
            window.removeEventListener('trash:drop', onTrashDrop);
        };
    }, [saveState]);
    ...
    return (_jsx("div", { ref: containerRef, style: {
            ...
            zIndex: 1,
            touchAction: 'none',
        }, children: _jsx("svg", { ref: svgRef, width: "100%", height: "100%", viewBox: `${viewBox.x} ${viewBox.y} ${viewBox.width} ${viewBox.height}`, preserveAspectRatio: "xMidYMid meet", onClick: handleCanvasClick, onMouseDown: handleMouseDown, onMouseMove: handleMouseMove, onMouseUp: handleMouseUp, onMouseLeave: handleMouseUp, onWheel: handleWheel, style: {
                display: 'block',
                cursor: getCursor(),
                width: '100%',
                height: '100%',
