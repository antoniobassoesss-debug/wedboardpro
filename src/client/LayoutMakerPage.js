...
const LayoutMakerPage = () => {
    ...
    const currentCanvasDataRef = useRef(null);
    // Ref to track which project the ref data belongs to
    const refProjectIdRef = useRef('1');
    ...
    const deepCopyCanvasData = useCallback((data) => {
        return {
    ...
    const handleProjectSelect = useCallback((projectId) => {
        ...
        if (currentCanvasDataRef.current &&
            ...
            setProjects(prevProjects => {
                const updated = prevProjects.map(project => project.id === activeProjectId
    ...
    const handleAddSpace = useCallback((widthMeters, heightMeters) => {
        if (gridCanvasRef.current) {
            gridCanvasRef.current.addSpace(widthMeters, heightMeters);
        }
    }, []);
    const handleAddTable = useCallback((type, size, seats, imageUrl, spaceId) => {
        if (gridCanvasRef.current) {
            gridCanvasRef.current.addTable(type, size, seats, imageUrl, spaceId);
        }
    }, []);
    const handleAddWalls = useCallback((walls, doors) => {
        if (gridCanvasRef.current) {
    ...
    const handleCanvasDataChange = useCallback((data, projectIdForData) => {
        ...
        setProjects(prevProjects => {
            const updated = prevProjects.map(project => project.id === projectIdForData
    ...
    return (_jsxs("div", { style: { position: 'relative', width: '100vw', height: '100vh', overflow: 'hidden' }, children: [_jsx(InfiniteGridBackground, {}), _jsx(GridCanvas, { ref: gridCanvasRef, activeTool: activeTool, onToolChange: setActiveTool, projectId: activeProjectId, projectData: deepCopyCanvasData(activeProject.canvasData), onDataChange: handleCanvasDataChange, brushSize: brushSize, brushColor: brushColor }), _jsxs("div", { style: {
                    position: 'fixed',
                    top: 0,
                    left: 0,
                    width: '100vw',
                    ...
                    zIndex: 10000,
                    isolation: 'isolate',
                }, children: [_jsx(HeaderBar, {}), _jsx(Toolbar, { activeTool: activeTool, onToolChange: setActiveTool, onAddSpace: handleAddSpace, onAddTable: handleAddTable, onAddWalls: handleAddWalls, brushSize: brushSize, brushColor: brushColor, onBrushSizeChange: setBrushSize, onBrushColorChange: setBrushColor, availableSpaces: spaceOptions }), _jsx(ProjectTabs, { projects: projects, activeProjectId: activeProjectId, onProjectSelect: handleProjectSelect, onNewProject: handleNewProject, onDeleteProject: handleDeleteProject, onRenameProject: handleRenameProject, newlyCreatedProjectId: newlyCreatedProjectId })] }), !isLoading && _jsx(AssistantChat, {}), isLoading && (_jsx("div", { style: {
                    position: 'fixed',
                    top: 0,
                    left: 0,
                    right: 0,
                    ...
                    zIndex: 20000,
                    pointerEvents: 'auto',
                }, children: _jsx("img", { src: "/loadinglogo.png", alt: "Loading", style: { width: '160px', height: 'auto', objectFit: 'contain' } }) }))] }));
};
export default LayoutMakerPage;
//# sourceMappingURL=LayoutMakerPage.js.map