...
export async function listCalendarEvents(params) {
    ...
    try {
        ...
        const { data: teamMember, error: teamError } = await browserSupabaseClient
            ...
            .single();
        let teamMemberIds = [params.currentUserId]; // Always include self
        ...
        if (params.from)
            privateQuery = privateQuery.gte('end_at', params.from);
        if (params.to)
            privateQuery = privateQuery.lte('start_at', params.to);
        if (params.projectId)
            privateQuery = privateQuery.eq('project_id', params.projectId);
        if (params.statuses && params.statuses.length > 0)
            privateQuery = privateQuery.in('status', params.statuses);
        if (params.eventTypes && params.eventTypes.length > 0)
            privateQuery = privateQuery.in('event_type', params.eventTypes);
        ...
        if (teamMemberIds.length > 0) {
            let teamQuery = browserSupabaseClient
                ...
                .order('start_at', { ascending: true });
            if (params.from)
                teamQuery = teamQuery.gte('end_at', params.from);
            if (params.to)
                teamQuery = teamQuery.lte('start_at', params.to);
            if (params.projectId)
                teamQuery = teamQuery.eq('project_id', params.projectId);
            if (params.statuses && params.statuses.length > 0)
                teamQuery = teamQuery.in('status', params.statuses);
            if (params.eventTypes && params.eventTypes.length > 0)
                teamQuery = teamQuery.in('event_type', params.eventTypes);
        ...
        const sharedEventIds = (sharedRows ?? []).map((r) => r.event_id);
        let customEvents = [];
        if (sharedEventIds.length > 0) {
            ...
            if (params.from)
                customQuery = customQuery.gte('end_at', params.from);
            if (params.to)
                customQuery = customQuery.lte('start_at', params.to);
            if (params.projectId)
                customQuery = customQuery.eq('project_id', params.projectId);
            if (params.statuses && params.statuses.length > 0)
                customQuery = customQuery.in('status', params.statuses);
            if (params.eventTypes && params.eventTypes.length > 0)
                customQuery = customQuery.in('event_type', params.eventTypes);
            const { data: customData, error: customErr } = await customQuery;
            if (customErr)
                return { data: null, error: customErr.message };
            customEvents = (customData ?? []);
        }
        // Step 5: Merge and dedupe
        const mergedMap = new Map();
        (privateEvents ?? []).forEach((ev) => mergedMap.set(ev.id, ev));
...
export async function createCalendarEvent(payload) {
    ...
    try {
        ...
        if (payload.visibility === 'custom' && payload.shared_user_ids && payload.shared_user_ids.length > 0) {
            const { error: sharedErr } = await browserSupabaseClient.from('calendar_event_shared_users').insert(payload.shared_user_ids.map((uid) => ({
                event_id: event.id,
                user_id: uid,
            })));
            if (sharedErr)
                return { data: null, error: sharedErr.message };
        }
...
//# sourceMappingURL=calendarEventsApi.js.map