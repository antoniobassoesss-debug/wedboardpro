    ...
    try {
        ...
        const completion = await openaiClient.chat.completions.create({
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an AI assistant for an event layout SaaS. Answer clearly and briefly. If the user asks about event layouts or seating, you may include structured JSON suggestions when appropriate.',
                },
...
app.post('/api/auth/signup', express.json(), async (req, res) => {
    ...
    try {
        ...
        console.log('/api/auth/signup supabase response:', {
            status: result,
            data: result?.data ? { ...(result.data), user: undefined } : undefined,
            error: result?.error,
        });
...
async function createTeamForUser(supabase, ownerId, name) {
...
async function getUserTeam(supabase, userId) {
    ...
    if (membershipRow?.teams) {
        return {
            ...
            membershipRole: membershipRow.role ?? 'member',
        };
    }
    ...
    if (ownerTeam) {
        return { team: ownerTeam, membershipRole: 'owner' };
    }
    return null;
}
async function ensureUserTeamMembership(supabase, userId) {
...
app.post('/api/teams', express.json(), async (req, res) => {
    ...
    const requestedName = typeof req.body?.name === 'string' && req.body.name.trim().length > 0 ? req.body.name.trim() : undefined;
    try {
...
app.get('/api/teams/members', express.json(), async (req, res) => {
    ...
    try {
        ...
        const enriched = membersSafe.map((m) => ({
            ...
            displayName: profilesById[m.user_id]?.full_name ??
...
app.get('/api/team', async (req, res) => {
    ...
    try {
        ...
        const members = membersSafe.map((m) => {
            ...
            const upcomingEventsCount = memberAssignments.filter((a) => {
                const ev = eventsById[a.event_id];
                return ev && isFutureOrToday(ev.wedding_date);
            }).length;
            const memberTasks = tasksByUserId[m.user_id] ?? [];
            const openTasksCount = memberTasks.filter((t) => t.status !== 'done').length;
            return {
                id: m.id,
                user_id: m.user_id,
                team_id: m.team_id,
...
app.get('/api/team/:id', async (req, res) => {
    ...
    try {
        ...
        const uniqueEventIds = eventIdsFromAssignments.filter((id, index, arr) => id && arr.indexOf(id) === index);
        let eventsById = {};
        ...
        const uniqueTaskEventIds = eventIdsFromTasks.filter((id, index, arr) => id && arr.indexOf(id) === index);
...
app.post('/api/teams/invite', express.json(), async (req, res) => {
    ...
    try {
        ...
        const { data: existingInvite } = await supabase
            ...
            .eq('status', 'pending')
            .maybeSingle();
        const generateToken = () => Buffer.from(Math.random().toString(36) + Date.now().toString(36)).toString('base64url').slice(0, 48);
...
app.get('/api/teams/invitations/pending', express.json(), async (req, res) => {
    ...
    try {
        ...
        const responseInvites = invitations?.map((invite) => ({
            ...invite,
...
app.get('/api/chat/messages', express.json(), async (req, res) => {
    ...
    const { limit: rawLimit, before, recipientId } = req.query ?? {};
...
async function logEventActivity(supabase, eventId, userId, action) {
    try {
...
app.post('/api/events', express.json(), async (req, res) => {
    try {
        ...
        const { title, wedding_date, guest_count_expected, guest_count_confirmed, budget_planned, budget_actual, notes_internal, } = req.body ?? {};
...
app.get('/api/events/:id', async (req, res) => {
    try {
        ...
        const [stagesResult, tasksResult, clientResult, venueResult, vendorsResult, filesResult, activityResult,] = await Promise.all([
...
app.patch('/api/events/:id', express.json(), async (req, res) => {
    try {
        ...
        if (body.current_stage !== undefined)
            patch.current_stage = body.current_stage;
        if (body.guest_count_expected !== undefined)
            patch.guest_count_expected = body.guest_count_expected;
        if (body.guest_count_confirmed !== undefined)
            patch.guest_count_confirmed = body.guest_count_confirmed;
        if (body.budget_planned !== undefined)
            patch.budget_planned = body.budget_planned;
        if (body.budget_actual !== undefined)
            patch.budget_actual = body.budget_actual;
        if (body.notes_internal !== undefined)
            patch.notes_internal = body.notes_internal;
...
app.patch('/api/stages/:id', express.json(), async (req, res) => {
    try {
        ...
        if (body.is_blocking !== undefined)
            patch.is_blocking = body.is_blocking;
...
app.patch('/api/vendors/:id', express.json(), async (req, res) => {
    try {
        ...
        if (body.contact_phone !== undefined)
            patch.contact_phone = body.contact_phone;
        ...
        if (body.quote_amount !== undefined)
            patch.quote_amount = body.quote_amount;
        if (body.final_amount !== undefined)
            patch.final_amount = body.final_amount;
...
app.get('/api/suppliers', async (req, res) => {
    try {
        ...
        const favorite = typeof req.query.favorite === 'string' && req.query.favorite.toLowerCase() === 'true';
        let query = supabase
            .from('suppliers')
        ...
        let countsBySupplier = {};
        if (supplierIds.length > 0) {
            const { data: counts, error: countError } = await supabase
                .from('event_suppliers')
                .select('supplier_id, event_id');
            if (!countError && counts) {
                countsBySupplier = counts.reduce((acc, row) => {
                    if (!row.supplier_id)
                        return acc;
...
app.patch('/api/suppliers/:id', express.json(), async (req, res) => {
    try {
        ...
        if (body.notes !== undefined)
            patch.notes = body.notes;
        if (body.rating_internal !== undefined)
            patch.rating_internal = body.rating_internal;
...
app.get('/api/events/:id/suppliers', async (req, res) => {
    try {
      ...
      `)
        ...
        const eventSuppliers = (rows ?? []).map((row) => ({
            ...row,
...
app.patch('/api/event-suppliers/:id', express.json(), async (req, res) => {
    try {
        ...
        if (body.quoted_price !== undefined)
            patch.quoted_price = body.quoted_price;
...
app.patch('/api/tasks/:id', express.json(), async (req, res) => {
    try {
        ...
        if (req.body.is_flagged !== undefined)
            updates.is_flagged = req.body.is_flagged;
        if (req.body.due_date !== undefined)
            updates.due_date = req.body.due_date || null;
        if (req.body.assignee_id !== undefined)
            updates.assignee_id = req.body.assignee_id || null;
        const supabase = getSupabaseServiceClient();
        if (!supabase) {
            return res.status(500).json({ error: 'Database connection failed' });
        }
...
async function notifyTaskAssignment(supabase, assigneeId, taskTitle, taskId, isReassignment = false) {
    try {
        // Get assignee email
