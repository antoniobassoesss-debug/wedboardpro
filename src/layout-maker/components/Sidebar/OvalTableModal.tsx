...
interface OvalTableModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: OvalTableData) => void;
}

const SEAT_SIZE_CM = 45;
const SEAT_OFFSET_CM = 10;

export const OvalTableModal: React.FC<OvalTableModalProps> = ({
  isOpen,
  onClose,
  onSubmit,
}) => {
  ...
  const handleWidthChange = useCallback((value: number) => {
    setWidth(Math.max(80, Math.min(400, value)));
  }, []);

  const handleHeightChange = useCallback((value: number) => {
    setHeight(Math.max(50, Math.min(200, value)));
  }, []);
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <input
                ...
                onChange={(e) => handleWidthChange(parseFloat(e.target.value) || 80)}
                style={{
                  flex: 1,
                  height: '36px',
                  padding: '0 12px',
              ...
              <input
                ...
                onChange={(e) => handleHeightChange(parseFloat(e.target.value) || 50)}
                style={{
                  flex: 1,
                  height: '36px',
                  padding: '0 12px',
          ...
          <div style={{ marginBottom: '20px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <input
                type="number"
                value={seats}
                onChange={(e) => setSeats(Math.max(4, Math.min(20, parseInt(e.target.value) || 4)))}
                style={{
                  width: '60px',
                  height: '36px',
                  textAlign: 'center',
          ...
          <div style={{ marginBottom: '16px' }}>
            <div style={{
              ...
              <svg width="100%" height="100%" viewBox="0 0 200 130">
                <defs>
                  <pattern id="gridOval" width="10" height="10" patternUnits="userSpaceOnUse">
                    <path d="M 10 0 L 0 0 0 10" fill="none" stroke="#e5e7eb" strokeWidth="0.5" />
                  </pattern>
                </defs>
                <rect width="200" height="130" fill="url(#gridOval)" />

                <g transform="translate(100, 65)">
                  <ellipse
                    cx={0}
                    cy={0}
                    rx={Math.min(previewData.width * 0.4, 70)}
                    ry={Math.min(previewData.height * 0.4, 45)}
                    fill="none"
                    stroke="#1f2937"
                    strokeWidth="2"
                  />
                  ...
                  {Array.from({ length: Math.min(seats, 10) }).map((_, i) => {
                    ...
                    const tableRx = Math.min(previewData.width * 0.4, 70);
                    const tableRy = Math.min(previewData.height * 0.4, 45);
                    const seatRadius = Math.sqrt(
                  ...
                  <line
                    x1={-Math.min(previewData.width * 0.4, 70) + 8}
                    y1="0"
                    x2={Math.min(previewData.width * 0.4, 70) - 8}
                    y2="0"
                    stroke="#1f2937"
                    strokeWidth="1"
                  />
                  <line
                    x1="0"
                    y1={-Math.min(previewData.height * 0.4, 45) + 8}
                    x2="0"
                    y2={Math.min(previewData.height * 0.4, 45) - 8}
                    stroke="#1f2937"
                    strokeWidth="1"
                  />
                </g>
